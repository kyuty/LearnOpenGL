// 41_stencil_testing.cpp : 定义控制台应用程序的入口点。
//
...
int main()
{
	...
	*/
	...
	Shader shaderSingleColor("shaders/advanced.vert", "shaders/single_color.frag");
...
#pragma endregion
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);// 它会告诉OpenGL，无论何时，
											// 一个fragment模板值等于（GL_EQUAL）引用值1，
											// fragment就能通过测试被绘制了，否则就会被丢弃。
											//
											//glStencilFunc(GLenum func, GLint ref, GLuint mask)有三个参数：
											//	func：设置模板测试方程。
											//		这个测试方程应用到
											//		已经储存的模板值和glStencilFunc的ref值上，
											//		可用的选项是：GL_NEVER、GL_LEQUAL、GL_GREATER、GL_GEQUAL、GL_EQUAL、GL_NOTEQUAL、GL_ALWAYS。
											//		它们的语义和深度缓冲的方程相似。
											//	ref：指定模板测试的引用值。模板缓冲的内容会与这个值对比。
											//	mask：指定一个遮罩，在模板测试对比引用值和储存的模板值前，对它们进行与（and）操作。初始设置为1。
		glStencilMask(0x00);			//关闭模版测试
		glDisable(GL_DEPTH_TEST);		//关闭深度测试 目的仅仅是让我们清晰的看到上下左右所有边框
		shaderSingleColor.Use();
		GLfloat scale = 1.1;
		// Cubes
		glBindVertexArray(cubeVAO);
