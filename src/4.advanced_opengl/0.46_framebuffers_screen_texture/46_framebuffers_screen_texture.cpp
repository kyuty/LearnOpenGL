// 46_framebuffers_screen_texture.cpp : 定义控制台应用程序的入口点。
//
...
#pragma endregion
	...
	// Draw as wireframe
	//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);// 左侧展示了和深度测试教程中一样的输出结果，
												// 但是这次却是渲染到一个简单的四边形上的。
												// 如果我们以线框方式显示的话，那么显然，我们只是绘制了一个默认帧缓冲中单调的四边形。
												// 然而这有什么好处呢？好处就是我们现在可以自由的获取已经渲染场景中的任何像素，
												// 然后把它当作一个纹理图像了，我们可以在像素着色器中创建一些有意思的效果。
												// 所有这些有意思的效果统称为后处理特效。
